...
def main():
    ...
    # ---- Create CSV header ----
    with open("tactile_log.csv", "w") as f:
        f.write("frame_id,time,fx,fy,fz,mx,my,mz,depth_values,deformation_x_values,deformation_y_values\n")
    ...
    try:
        while dpg.is_dearpygui_running():
            ...
            # Save per-frame values in CSV: all depth and deformation X/Y values flattened
            if depth is not None and deformation is not None:
                def_x = deformation[..., 0]
                def_y = deformation[..., 1]
                depth_str = ";".join(f"{v:.6f}" for v in depth.ravel())
                def_x_str = ";".join(f"{v:.6f}" for v in def_x.ravel())
                def_y_str = ";".join(f"{v:.6f}" for v in def_y.ravel())
                with open("tactile_log.csv", "a") as f:
                    f.write(
